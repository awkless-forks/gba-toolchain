...
set(CMAKE_SYSTEM_NAME HomebrewGBA CACHE INTERNAL "")
...
if(CMAKE_HOST_SYSTEM_NAME MATCHES "Windows")
    if(CMAKE_GENERATOR MATCHES "Visual Studio")
        message(
            FATAL_ERROR
            "Toolchain is not compatible with Visual Studio (Use -G \"Ninja\" or -G \"Unix Makefiles\")"
        )
    endif()
    ...
    if(CMAKE_GENERATOR STREQUAL "NMake Makefiles")
        message(
            FATAL_ERROR
            "Toolchain is not compatible with NMake (Use -G \"Ninja\" or -G \"Unix Makefiles\")"
        )
    endif()

    # INVARIANT: Fixup devkitPro default environment paths for Windows.
    # FIXME: This is not guaranteed to produce a correct path for devkitPro.
    ...
    # INVARIANT: Find default install path for Arm GNU Toolchain.
    unset(programfiles)
...
# INVARIANT: Fixup MSYS search paths
if(CMAKE_HOST_SYSTEM_NAME STREQUAL "MSYS")
    # Disallow msys2 CMake
    if(CMAKE_COMMAND MATCHES "msys2")
        message(
            FATAL_ERROR
            "${CMAKE_COMMAND} is known to cause problems. Please use an alternative CMake executable."
        )
    endif()

    if(DEFINED "ENV{GNUARM}")
        execute_process(
            COMMAND cygpath -u "$ENV{GNUARM}"
            OUTPUT_VARIABLE GNUARM
    ...
    if(DEFINED "ENV{DEVKITPRO}")
        execute_process(
            COMMAND cygpath -u "$ENV{DEVKITPRO}"
            OUTPUT_VARIABLE DEVKITPRO
            OUTPUT_STRIP_TRAILING_WHITESPACE
        )
        set(ENV{DEVKITPRO} "${DEVKITPRO}")
        unset(DEVKITPRO)
    ...
    if(DEFINED "ENV{DEVKITARM}")
        execute_process(
            COMMAND cygpath -u "$ENV{DEVKITARM}"
            OUTPUT_VARIABLE DEVKITARM
            OUTPUT_STRIP_TRAILING_WHITESPACE
        )
        set(ENV{DEVKITARM} "${DEVKITARM}")
        unset(DEVKITARM)
...
set(COMPILER_SEARCH_PATHS
    "$ENV{GNUARM}"
    "$ENV{DEVKITARM}"
    "$ENV{DEVKITPRO}/devkitARM"
)

# INVARIANT: Set library prefixes and suffixes.
...
# INVARIANT: Set CMAKE_MAKE_PROGRAM for Unix Makefiles.
...
function(find_arm_compiler lang binary)
    ...
    if(CMAKE_${lang}_COMPILER)
        ...
        if(CMAKE_${lang}_COMPILER_ID MATCHES "Clang")
            execute_process(
                COMMAND ${CMAKE_${lang}_COMPILER} -print-targets
        ...
        elseif(CMAKE_${lang}_COMPILER_ID MATCHES "GNU")
            execute_process(
                COMMAND ${CMAKE_${lang}_COMPILER} -dumpmachine
...
# SAFETY: Use GCC for ASM to solve compiler flag woes for GNU AS.
find_arm_compiler(ASM arm-none-eabi-gcc)
find_arm_compiler(C arm-none-eabi-gcc)
find_arm_compiler(CXX arm-none-eabi-g++)

# INVARIANT: Set compiler target triples
...
# INVARIANT: Find C compiler in sysroot.
find_program(
    SYSROOT_COMPILER
...
# INVARIANT: Find sysroot top-level directory.
...
# INVARIANT: Check for nano libs.
execute_process(
    COMMAND "${CMAKE_C_COMPILER}" -dumpversion
    OUTPUT_VARIABLE LIBGCC_VERSION
    OUTPUT_STRIP_TRAILING_WHITESPACE
)

find_path(
    LIBGCC_DIRECTORY
    "gcc/arm-none-eabi/${LIBGCC_VERSION}"
    PATHS "${SYSROOT_DIRECTORY}"
    PATH_SUFFIXES lib
)
...
find_library(
    ...
    stdc++_nano
    supc++_nano
    PATHS "${LIBGCC_DIRECTORY}/gcc/arm-none-eabi/${LIBGCC_VERSION}"
)
...
# INVARIANT: Set __DEVKITARM__ macro.
execute_process(
...
# INVARIANT: Setup default linker flags.
execute_process(
...
if(LD_FLAGS MATCHES "[-][-]no[-]warn[-]rwx[-]segments")
    set(CMAKE_EXE_LINKER_FLAGS
        "-Wl,--no-warn-rwx-segments -nostartfiles"
        CACHE INTERNAL
        ""
    )
else()
...
# INVARIANT: Set system prefix path.
...
set(CMAKE_SYSTEM_LIBRARY_PATH
    "${CMAKE_SYSTEM_PREFIX_PATH}/gba-toolchain/lib"
    CACHE INTERNAL
    ""
)
